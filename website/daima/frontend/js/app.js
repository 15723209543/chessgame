...
async function qilei_load_admin() {
  ...
  try {
    ...
    for (const qilei_user of qilei_users) {
      ...
      const qilei_role_text = qilei_user.is_admin
        ? "管理员"
        : (qilei_user.account_ready ? "普通用户" : "普通用户（待设置密码）"); // qilei_role_text：管理员、正常用户或缺少账号文件用户的身份提示。
      for (const qilei_text of [qilei_user.username, qilei_role_text, qilei_format_date(qilei_user.created_at), String(qilei_user.opened_games), String(qilei_user.completed_games)]) { const qilei_cell = document.createElement("td"); qilei_cell.textContent = qilei_text; qilei_row.append(qilei_cell); }
      ...
      qilei_password_button.textContent = qilei_user.account_ready ? "修改密码" : "设置密码";
