...
- 删除全部历史；
- 管理员统计；
- 扫描并显示缺少账号 JSON 的已有数字玩家目录；
- 管理员为缺少账号 JSON 的玩家设置密码后补齐账号；
- 删除用户目录；
...
- 围棋引擎常驻复用；
- 前端后备机器人；
- 跳棋终点区按到达者映射颜色，并执行六成保护、外来棋子撤离和违规判负规则；
- 跳棋局势分析按推进、配合、终点和桥梁评分精确归一化为 100%；
- 跳棋的合法目标统一显示绿色轮廓，前九项使用 1–9，第十项起使用 A–Z，并在右侧可走位置提示中保持一致；
- 网页象棋把空目标显示为绿色点、可吃棋子显示为绿色外圈，二者均沿用二次确认流程；
- 异步结果会话隔离；
- CSP、路径边界和请求大小限制；
- PyInstaller 本地启动器。
