...
#include "tiaoqi_drawer.h"
#include "tiaoqi_keyboard.h"
#include "tiaoqi_keyboard_data.h"
#include "tiaoqi_logger.h"
#include "tiaoqi_mapdata.h"
#include "tiaoqi_robot.h"
...
static void game_refresh_buttons(gamestate& state)
{
    ...
    if (state.phase == phase_player_count || state.phase == phase_piece_count)
    {
        ...
        state.buttons.push_back(game_make_button(map_setup_exit_button_y, button_exit, 0, L"返回主界面"));
    }
    else if (state.phase == phase_select_piece || state.phase == phase_select_target)
    {
        ...
        state.buttons.push_back(game_make_button(map_bottom_button_second_y, button_exit, 0, L"返回主界面"));
    }
...
{
    if (key == keyboard_key_escape && game_is_setup_phase(state))
    {
        state.running = false;
        logger_write(logger, L"设置页按 ESC 返回主界面");
        return;
    }
