...
#include "zhongguoxiangqi_robot.h"
                            ...
                            const optionbutton& backbutton, const optionbutton& startbutton, int activegroup)
{
    ...
    drawsettingtext(backbutton.left, backbutton.top, backbutton.right, backbutton.bottom, L"返回主界面", RGB(255, 255, 255), 24);
    ...
    drawsettingtext(0, window_height - 62, window_width - 1, window_height - 34, L"点击返回主界面或按 ESC 返回，按 Enter 开始", RGB(83, 90, 69), 18);
    EndBatchDraw();
}
...
bool choosegamesetting(timesetting& setting, robotsetting& robots)
{
    ...
    drawgamesetting(setting, robots, buttons, backbutton, startbutton, activegroup);
    ...
    while (true)
    {
        ...
        if (message.message == WM_LBUTTONDOWN)
        {
            ...
            if (inoptionbutton(message.x, message.y, backbutton))
            {
                return false;
            }
