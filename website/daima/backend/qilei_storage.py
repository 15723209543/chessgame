...
class qilei_json_storage:
    ...
    def qilei_user_folder_exists(self, qilei_username: str) -> bool:
        """确认纯数字玩家目录真实存在并且没有越出 data 根目录。"""
        if not qilei_username.isdigit():
            return False
        # qilei_target：解析后的待检查玩家目录。
        qilei_target = self.qilei_user_dir(qilei_username).resolve()
        # qilei_data_root：解析后的 data 根目录，用于校验目录边界。
        qilei_data_root = self.qilei_data_dir.resolve()
        return qilei_target.parent == qilei_data_root and qilei_target.is_dir()
    ...
    def qilei_admin_users(self) -> List[Dict[str, Any]]:
        """为管理员汇总每个数字玩家目录、账号状态与单局文件数量。"""
        with self.qilei_lock:
            # qilei_accounts：从各 user 文件夹扫描到的全部账号摘要。
            qilei_accounts = self.qilei_load_accounts()
            # qilei_usernames：账号文件和 data 下数字玩家目录的用户名并集。
            qilei_usernames = set(qilei_accounts)
            qilei_usernames.update(
                qilei_path.name
                for qilei_path in self.qilei_data_dir.iterdir()
                if qilei_path.is_dir() and qilei_path.name.isdigit()
            )
            # qilei_result：不含密码摘要的管理员账号概览。
            qilei_result = []
            for qilei_username in sorted(qilei_usernames):
                # qilei_account：可能为空的当前玩家账号摘要。
                qilei_account = qilei_accounts.get(qilei_username, {})
                # qilei_account_ready：盐和密码摘要都存在时表示账号可以正常登录。
                qilei_account_ready = bool(qilei_account.get("salt") and qilei_account.get("password_hash"))
                # qilei_created_at：优先使用账号注册时间，缺失时使用玩家目录创建时间。
                qilei_created_at = qilei_account.get("created_at", "")
                if not qilei_created_at:
                    try:
                        qilei_created_at = datetime.fromtimestamp(
                            self.qilei_user_dir(qilei_username).stat().st_ctime
                ...
                qilei_result.append({
                    ...
                    "account_ready": qilei_account_ready,
                    "created_at": qilei_created_at,
