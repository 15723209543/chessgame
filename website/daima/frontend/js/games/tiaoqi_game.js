...
// tiaoqi_reorder_camp：把一个三角阵营按真实角尖到棋盘中心的层级重新排列。
function tiaoqi_reorder_camp(tiaoqi_camp, tiaoqi_arm) {
  const tiaoqi_tip = tiaoqi_camp.reduce((tiaoqi_best, tiaoqi_index) => {
    if (tiaoqi_best < 0) return tiaoqi_index;
    const tiaoqi_point = tiaoqi_points[tiaoqi_index]; // tiaoqi_point：当前候选角尖孔位。
    const tiaoqi_best_point = tiaoqi_points[tiaoqi_best]; // tiaoqi_best_point：此前最靠近角尖的孔位。
    if (tiaoqi_arm === 0) return tiaoqi_point.y < tiaoqi_best_point.y ? tiaoqi_index : tiaoqi_best;
    if (tiaoqi_arm === 3) return tiaoqi_point.y > tiaoqi_best_point.y ? tiaoqi_index : tiaoqi_best;
    if (tiaoqi_arm === 1 || tiaoqi_arm === 2) return tiaoqi_point.x > tiaoqi_best_point.x ? tiaoqi_index : tiaoqi_best;
    return tiaoqi_point.x < tiaoqi_best_point.x ? tiaoqi_index : tiaoqi_best;
  }, -1); // tiaoqi_tip：该阵营真实角尖孔位。
  const tiaoqi_distances = new Map([[tiaoqi_tip, 0]]); // tiaoqi_distances：阵营内部从角尖出发的相邻步数。
  const tiaoqi_queue = [tiaoqi_tip]; // tiaoqi_queue：等待展开的阵营孔位。
  while (tiaoqi_queue.length) {
    const tiaoqi_current = tiaoqi_queue.shift(); // tiaoqi_current：当前展开的阵营孔位。
    for (const tiaoqi_next of tiaoqi_camp) {
      if (tiaoqi_distances.has(tiaoqi_next)) continue;
      if (Math.abs(tiaoqi_distance(tiaoqi_points[tiaoqi_current], tiaoqi_points[tiaoqi_next]) - tiaoqi_point_space) > 1.1) continue;
      tiaoqi_distances.set(tiaoqi_next, tiaoqi_distances.get(tiaoqi_current) + 1);
      tiaoqi_queue.push(tiaoqi_next);
    }
  }
  return [...tiaoqi_camp].sort((tiaoqi_left, tiaoqi_right) => {
    const tiaoqi_layer_difference = (tiaoqi_distances.get(tiaoqi_left) ?? 99) - (tiaoqi_distances.get(tiaoqi_right) ?? 99); // tiaoqi_layer_difference：两孔位从角尖向外的层数差。
    if (tiaoqi_layer_difference !== 0) return tiaoqi_layer_difference;
    const tiaoqi_y_difference = tiaoqi_points[tiaoqi_left].y - tiaoqi_points[tiaoqi_right].y; // tiaoqi_y_difference：同层孔位纵坐标差。
    return Math.abs(tiaoqi_y_difference) > 0.1 ? tiaoqi_y_difference : tiaoqi_points[tiaoqi_left].x - tiaoqi_points[tiaoqi_right].x;
  });
}
...
function tiaoqi_build_camps() {
  ...
  return [tiaoqi_top, tiaoqi_upper_right, tiaoqi_lower_right, tiaoqi_bottom, tiaoqi_lower_left, tiaoqi_upper_left]
    .map((tiaoqi_camp, tiaoqi_arm) => tiaoqi_reorder_camp(tiaoqi_camp, tiaoqi_arm));
}
...
function tiaoqi_restore_state(tiaoqi_value) {
  if (tiaoqi_valid_state(tiaoqi_value)) {
    ...
    tiaoqi_restored.eliminationReason = Array.isArray(tiaoqi_restored.eliminationReason) && tiaoqi_restored.eliminationReason.length === tiaoqi_restored.settings.playerCount
      ? tiaoqi_restored.eliminationReason
      : Array(tiaoqi_restored.settings.playerCount).fill("");
...
// tiaoqi_zone_layer：返回孔位在三角区域中从角尖向出口计算的层数。
function tiaoqi_zone_layer(tiaoqi_zone, tiaoqi_index) {
  const tiaoqi_position = tiaoqi_zone.indexOf(tiaoqi_index); // tiaoqi_position：孔位在按层排列区域中的下标。
  if (tiaoqi_position < 0) return -1;
  let tiaoqi_layer = 0; // tiaoqi_layer：当前三角层号。
  let tiaoqi_layer_end = 1; // tiaoqi_layer_end：当前层结束后的首个下标。
  while (tiaoqi_position >= tiaoqi_layer_end) {
    tiaoqi_layer += 1;
    tiaoqi_layer_end += tiaoqi_layer + 1;
  }
  return tiaoqi_layer;
}

// tiaoqi_point_allowed_for_player：禁止外来棋子停入他人终点区，并只允许原有外来棋子向出口撤离。
function tiaoqi_point_allowed_for_player(tiaoqi_state, tiaoqi_player, tiaoqi_start, tiaoqi_target) {
  for (let tiaoqi_zone_owner = 0; tiaoqi_zone_owner < tiaoqi_state.settings.playerCount; tiaoqi_zone_owner += 1) {
    if (tiaoqi_zone_owner === tiaoqi_player) continue;
    const tiaoqi_zone = tiaoqi_goal_camp(tiaoqi_state, tiaoqi_zone_owner); // tiaoqi_zone：另一名玩家的最终区域。
    if (!tiaoqi_zone.includes(tiaoqi_target)) continue;
    const tiaoqi_start_layer = tiaoqi_zone_layer(tiaoqi_zone, tiaoqi_start); // tiaoqi_start_layer：原位置在该区域中的层号。
    const tiaoqi_target_layer = tiaoqi_zone_layer(tiaoqi_zone, tiaoqi_target); // tiaoqi_target_layer：候选落点在该区域中的层号。
    return tiaoqi_start_layer >= 0 && tiaoqi_target_layer > tiaoqi_start_layer;
  }
  return true;
}

// tiaoqi_goal_path_allowed：己方棋子进入自己的最终区域后只能继续留在区域内。
function tiaoqi_goal_path_allowed(tiaoqi_state, tiaoqi_player, tiaoqi_current, tiaoqi_target) {
  const tiaoqi_goal = tiaoqi_goal_camp(tiaoqi_state, tiaoqi_player); // tiaoqi_goal：当前玩家自己的最终区域。
  return !tiaoqi_goal.includes(tiaoqi_current) || tiaoqi_goal.includes(tiaoqi_target);
}

// tiaoqi_jump_targets：按本地 C++ 规则广度搜索一次或连续多次跳跃的全部可达空孔。
function tiaoqi_jump_targets(tiaoqi_state, tiaoqi_player, tiaoqi_from) {
  ...
  while (tiaoqi_queue.length) {
    ...
    for (const tiaoqi_jump of tiaoqi_links.jumps[tiaoqi_current]) {
      const tiaoqi_middle_occupied = tiaoqi_jump.middle === tiaoqi_current
        || (tiaoqi_jump.middle !== tiaoqi_from && Boolean(tiaoqi_state.board[tiaoqi_jump.middle])); // tiaoqi_middle_occupied：连续跳模拟局面中的跨越孔占用状态。
      const tiaoqi_target_empty = tiaoqi_jump.to !== tiaoqi_from
        && tiaoqi_jump.to !== tiaoqi_current
        && !tiaoqi_state.board[tiaoqi_jump.to]; // tiaoqi_target_empty：连续跳模拟局面中的落点空闲状态。
      if (!tiaoqi_middle_occupied || !tiaoqi_target_empty || tiaoqi_seen.has(tiaoqi_jump.to)) continue;
      if (!tiaoqi_goal_path_allowed(tiaoqi_state, tiaoqi_player, tiaoqi_current, tiaoqi_jump.to)) continue;
      tiaoqi_seen.add(tiaoqi_jump.to);
      tiaoqi_queue.push(tiaoqi_jump.to);
      if (tiaoqi_point_allowed_for_player(tiaoqi_state, tiaoqi_player, tiaoqi_from, tiaoqi_jump.to)) {
...
// tiaoqi_legal_targets：按本地 C++ 的终点区保护规则合并相邻步进和连续跳跃落点。
function tiaoqi_legal_targets(tiaoqi_state, tiaoqi_player, tiaoqi_from) {
  const tiaoqi_targets = new Set(tiaoqi_jump_targets(tiaoqi_state, tiaoqi_player, tiaoqi_from)); // tiaoqi_targets：当前棋子的全部合法目标。
  for (const tiaoqi_to of tiaoqi_links.neighbors[tiaoqi_from]) {
    if (tiaoqi_state.board[tiaoqi_to]) continue;
    if (!tiaoqi_goal_path_allowed(tiaoqi_state, tiaoqi_player, tiaoqi_from, tiaoqi_to)) continue;
    if (!tiaoqi_point_allowed_for_player(tiaoqi_state, tiaoqi_player, tiaoqi_from, tiaoqi_to)) continue;
    tiaoqi_targets.add(tiaoqi_to);
  }
  return [...tiaoqi_targets];
}
...
function tiaoqi_goal_camp(tiaoqi_state, tiaoqi_player) {
  ...
  const tiaoqi_goal = tiaoqi_camps[(tiaoqi_arm + 3) % 6] || []; // tiaoqi_goal：对面完整三角阵营。
  return tiaoqi_goal.slice(0, tiaoqi_state.settings.pieceCount);
}
...
// tiaoqi_analysis_piece_progress：按本地 C++ 的最远起点与目标参考点计算一枚棋子的推进比例。
function tiaoqi_analysis_piece_progress(tiaoqi_state, tiaoqi_player, tiaoqi_index) {
  const tiaoqi_start = (tiaoqi_camps[tiaoqi_state.playerArms[tiaoqi_player]] || []).slice(0, tiaoqi_state.settings.pieceCount); // tiaoqi_start：玩家实际使用的起始区域。
  const tiaoqi_goal = tiaoqi_goal_camp(tiaoqi_state, tiaoqi_player); // tiaoqi_goal：玩家实际使用的最终区域。
  if (!tiaoqi_start.length || !tiaoqi_goal.length) return 0;
  let tiaoqi_best_start = tiaoqi_start[0]; // tiaoqi_best_start：与某个目标孔距离最远的起点。
  let tiaoqi_best_target = tiaoqi_goal[0]; // tiaoqi_best_target：最远参考距离对应的目标孔。
  let tiaoqi_max_distance = 1; // tiaoqi_max_distance：起点到终点的最大参考距离。
  for (const tiaoqi_start_index of tiaoqi_start) {
    for (const tiaoqi_goal_index of tiaoqi_goal) {
      const tiaoqi_reference_distance = tiaoqi_distance(tiaoqi_points[tiaoqi_start_index], tiaoqi_points[tiaoqi_goal_index]); // tiaoqi_reference_distance：一对起点和目标孔的距离。
      if (tiaoqi_reference_distance <= tiaoqi_max_distance) continue;
      tiaoqi_max_distance = tiaoqi_reference_distance;
      tiaoqi_best_start = tiaoqi_start_index;
      tiaoqi_best_target = tiaoqi_goal_index;
    }
  }
  const tiaoqi_start_distance = tiaoqi_distance(tiaoqi_points[tiaoqi_best_start], tiaoqi_points[tiaoqi_best_target]); // tiaoqi_start_distance：参考起点到参考终点的完整距离。
  if (tiaoqi_start_distance <= 1) return 0;
  const tiaoqi_current_distance = tiaoqi_distance(tiaoqi_points[tiaoqi_index], tiaoqi_points[tiaoqi_best_target]); // tiaoqi_current_distance：当前棋子到参考终点的距离。
  return qilei_clamp(1 - tiaoqi_current_distance / tiaoqi_start_distance, 0, 1);
}

// tiaoqi_analysis_has_cooperate_jump：判断棋子是否能借助同方棋子直接跳跃。
function tiaoqi_analysis_has_cooperate_jump(tiaoqi_state, tiaoqi_player, tiaoqi_index) {
  return tiaoqi_links.jumps[tiaoqi_index].some(tiaoqi_jump => (
    tiaoqi_state.board[tiaoqi_jump.middle] === tiaoqi_player + 1
    && !tiaoqi_state.board[tiaoqi_jump.to]
  ));
}

// tiaoqi_analysis_bridge_count：统计一枚棋子为其他玩家提供的直接跳跃桥梁数量。
function tiaoqi_analysis_bridge_count(tiaoqi_state, tiaoqi_player, tiaoqi_bridge_index) {
  let tiaoqi_count = 0; // tiaoqi_count：其他玩家可借此棋子完成的跳跃数量。
  for (let tiaoqi_other_player = 0; tiaoqi_other_player < tiaoqi_state.settings.playerCount; tiaoqi_other_player += 1) {
    if (tiaoqi_other_player === tiaoqi_player || tiaoqi_state.eliminated?.[tiaoqi_other_player]) continue;
    for (const tiaoqi_other_index of tiaoqi_piece_indices(tiaoqi_state, tiaoqi_other_player)) {
      for (const tiaoqi_jump of tiaoqi_links.jumps[tiaoqi_other_index]) {
        if (tiaoqi_jump.middle === tiaoqi_bridge_index && !tiaoqi_state.board[tiaoqi_jump.to]) tiaoqi_count += 1;
      }
    }
  }
  return tiaoqi_count;
...
// tiaoqi_analysis_player_score：复刻本地 C++ 的推进、配合、终点和桥梁评分。
function tiaoqi_analysis_player_score(tiaoqi_state, tiaoqi_player) {
  if (tiaoqi_state.eliminated?.[tiaoqi_player] || !tiaoqi_piece_indices(tiaoqi_state, tiaoqi_player).length) return 0;
  let tiaoqi_score = 100; // tiaoqi_score：该玩家的基础分和全部加分。
  let tiaoqi_penalty = 0; // tiaoqi_penalty：给其他玩家提供桥梁产生的扣分。
  const tiaoqi_goal = tiaoqi_goal_camp(tiaoqi_state, tiaoqi_player); // tiaoqi_goal：该玩家最终区域。
  for (const tiaoqi_index of tiaoqi_piece_indices(tiaoqi_state, tiaoqi_player)) {
    const tiaoqi_progress = tiaoqi_analysis_piece_progress(tiaoqi_state, tiaoqi_player, tiaoqi_index); // tiaoqi_progress：当前棋子的推进比例。
    if (tiaoqi_progress > 0.5) tiaoqi_score += Math.floor((tiaoqi_progress - 0.5) * 2 * 28);
    if (tiaoqi_analysis_has_cooperate_jump(tiaoqi_state, tiaoqi_player, tiaoqi_index)) tiaoqi_score += 16;
    if (tiaoqi_goal.includes(tiaoqi_index)) tiaoqi_score += 60;
    tiaoqi_penalty += tiaoqi_analysis_bridge_count(tiaoqi_state, tiaoqi_player, tiaoqi_index) * 10;
  }
  return Math.max(0, tiaoqi_score - tiaoqi_penalty);
}

// tiaoqi_analysis_percentages：把本地 C++ 同款评分折算成总和严格等于一百的整数胜率。
function tiaoqi_analysis_percentages(tiaoqi_state) {
  const tiaoqi_scores = Array.from({ length: tiaoqi_state.settings.playerCount }, (_, tiaoqi_player) => tiaoqi_analysis_player_score(tiaoqi_state, tiaoqi_player)); // tiaoqi_scores：所有玩家当前局势评分。
  const tiaoqi_total = tiaoqi_scores.reduce((tiaoqi_sum, tiaoqi_value) => tiaoqi_sum + tiaoqi_value, 0); // tiaoqi_total：有效玩家评分总和。
  if (tiaoqi_total <= 0) return tiaoqi_scores.map(() => 0);
  let tiaoqi_last_positive = -1; // tiaoqi_last_positive：接收四舍五入余数的最后一名有效玩家。
  tiaoqi_scores.forEach((tiaoqi_score, tiaoqi_player) => {
    if (tiaoqi_score > 0) tiaoqi_last_positive = tiaoqi_player;
  });
  let tiaoqi_used = 0; // tiaoqi_used：最后一名玩家之前已经分配的百分比。
  return tiaoqi_scores.map((tiaoqi_score, tiaoqi_player) => {
    if (tiaoqi_score <= 0) return 0;
    if (tiaoqi_player === tiaoqi_last_positive) return 100 - tiaoqi_used;
    const tiaoqi_percent = Math.floor(tiaoqi_score * 100 / tiaoqi_total + 0.5); // tiaoqi_percent：当前玩家四舍五入后的整数百分比。
    tiaoqi_used += tiaoqi_percent;
    return tiaoqi_percent;
  });
}

// tiaoqi_goal_owner_for_arm：返回把指定外臂作为最终区域的玩家编号。
function tiaoqi_goal_owner_for_arm(tiaoqi_state, tiaoqi_arm) {
  return tiaoqi_state.playerArms.findIndex(tiaoqi_start_arm => (tiaoqi_start_arm + 3) % 6 === tiaoqi_arm);
}

// tiaoqi_zone_protection_count：返回最终区域启动保护所需的己方棋子数。
function tiaoqi_zone_protection_count(tiaoqi_state) {
  return Math.floor((tiaoqi_state.settings.pieceCount * 6 + 9) / 10);
}

// tiaoqi_protected_zone_violators：查找已经受保护终点区内仍然滞留的全部外来玩家。
function tiaoqi_protected_zone_violators(tiaoqi_state) {
  const tiaoqi_violators = new Set(); // tiaoqi_violators：需要立即判负的玩家集合。
  const tiaoqi_threshold = tiaoqi_zone_protection_count(tiaoqi_state); // tiaoqi_threshold：本局六成保护阈值。
  for (let tiaoqi_zone_owner = 0; tiaoqi_zone_owner < tiaoqi_state.settings.playerCount; tiaoqi_zone_owner += 1) {
    if (tiaoqi_state.eliminated?.[tiaoqi_zone_owner] || tiaoqi_goal_count(tiaoqi_state, tiaoqi_zone_owner) < tiaoqi_threshold) continue;
    const tiaoqi_zone = new Set(tiaoqi_goal_camp(tiaoqi_state, tiaoqi_zone_owner)); // tiaoqi_zone：已进入保护状态的最终区域。
    for (const tiaoqi_index of tiaoqi_zone) {
      const tiaoqi_owner = (tiaoqi_state.board[tiaoqi_index] || 0) - 1; // tiaoqi_owner：当前占据保护区孔位的玩家。
      if (tiaoqi_owner >= 0
        && tiaoqi_owner !== tiaoqi_zone_owner
        && !tiaoqi_state.eliminated?.[tiaoqi_owner]
        && !tiaoqi_state.completed?.[tiaoqi_owner]) {
        tiaoqi_violators.add(tiaoqi_owner);
      }
    }
  }
  return [...tiaoqi_violators];
}

// tiaoqi_opponent_zone_risk：计算一个孔位位于其他玩家最终区时的撤离风险。
function tiaoqi_opponent_zone_risk(tiaoqi_state, tiaoqi_player, tiaoqi_index) {
  let tiaoqi_risk = 0; // tiaoqi_risk：零为安全，一至三表示逐步升高的滞留风险。
  const tiaoqi_warning_count = Math.floor((tiaoqi_state.settings.pieceCount * 3 + 9) / 10); // tiaoqi_warning_count：机器人开始强制撤离时的三成阈值。
  for (let tiaoqi_zone_owner = 0; tiaoqi_zone_owner < tiaoqi_state.settings.playerCount; tiaoqi_zone_owner += 1) {
    if (tiaoqi_zone_owner === tiaoqi_player || !tiaoqi_goal_camp(tiaoqi_state, tiaoqi_zone_owner).includes(tiaoqi_index)) continue;
    const tiaoqi_finished = tiaoqi_goal_count(tiaoqi_state, tiaoqi_zone_owner); // tiaoqi_finished：区域主人已经进入终点的棋子数。
    const tiaoqi_current_risk = tiaoqi_finished >= tiaoqi_warning_count ? 3 : (tiaoqi_finished + 1 >= tiaoqi_warning_count ? 2 : 1); // tiaoqi_current_risk：该终点区的当前风险等级。
    tiaoqi_risk = Math.max(tiaoqi_risk, tiaoqi_current_risk);
  }
  return tiaoqi_risk;
}

// tiaoqi_opponent_zone_layer：返回孔位在任一对方最终区中的最深层号。
function tiaoqi_opponent_zone_layer(tiaoqi_state, tiaoqi_player, tiaoqi_index) {
  let tiaoqi_result = -1; // tiaoqi_result：最靠近角尖的区域层号。
  for (let tiaoqi_zone_owner = 0; tiaoqi_zone_owner < tiaoqi_state.settings.playerCount; tiaoqi_zone_owner += 1) {
    if (tiaoqi_zone_owner === tiaoqi_player) continue;
    const tiaoqi_layer = tiaoqi_zone_layer(tiaoqi_goal_camp(tiaoqi_state, tiaoqi_zone_owner), tiaoqi_index); // tiaoqi_layer：当前孔位在该对手终点区的层号。
    if (tiaoqi_layer >= 0 && (tiaoqi_result < 0 || tiaoqi_layer < tiaoqi_result)) tiaoqi_result = tiaoqi_layer;
  }
  return tiaoqi_result;
}
...
function tiaoqi_all_moves(tiaoqi_state, tiaoqi_player) {
  ...
  for (const tiaoqi_from of tiaoqi_piece_indices(tiaoqi_state, tiaoqi_player)) {
    for (const tiaoqi_to of tiaoqi_legal_targets(tiaoqi_state, tiaoqi_player, tiaoqi_from)) {
...
// tiaoqi_elimination_label：把内部判负原因转换为对局结束后保持不变的简短状态。
function tiaoqi_elimination_label(tiaoqi_state, tiaoqi_player) {
  const tiaoqi_reason = tiaoqi_state.eliminationReason?.[tiaoqi_player] || ""; // tiaoqi_reason：该玩家被判负时记录的原始原因。
  if (tiaoqi_reason.includes("耗尽") || tiaoqi_reason.includes("超时")) return "超时出局";
  if (tiaoqi_reason.includes("终点区")) return "终点区未及时离开判负";
  if (tiaoqi_reason.includes("合法走法")) return "无合法走法判负";
  return tiaoqi_reason || "判负出局";
}
...
export function tiaoqi_create_game({ boardHost: tiaoqi_board_host, controlsHost: tiaoqi_controls_host, savedState: tiaoqi_saved_state, services: tiaoqi_services }) {
    ...
    tiaoqi_clock_timer = window.setInterval(() => {
      ...
      tiaoqi_controls_host.querySelectorAll(".tiaoqi-player-detail").forEach(tiaoqi_node => {
        ...
        if (tiaoqi_state.eliminated?.[tiaoqi_player]) {
          tiaoqi_node.textContent = `${tiaoqi_elimination_label(tiaoqi_state, tiaoqi_player)}  第${tiaoqi_eliminated_rank(tiaoqi_state, tiaoqi_player)}名  完成${tiaoqi_state.eliminationProgress?.[tiaoqi_player] || 0}/${tiaoqi_state.settings.pieceCount}`;
  ...
  function tiaoqi_start_game() {
    ...
    tiaoqi_state.eliminationReason = Array(tiaoqi_state.settings.playerCount).fill("");
  ...
  // tiaoqi_target_label：把零开始目标下标转换为棋盘和提示区共用的 1-9、A-Z 标签。
  function tiaoqi_target_label(tiaoqi_target_index) {
    if (tiaoqi_target_index < 0 || tiaoqi_target_index >= 35) return "";
    if (tiaoqi_target_index < 9) return String(tiaoqi_target_index + 1);
    return String.fromCharCode("A".charCodeAt(0) + tiaoqi_target_index - 9);
  }

  // tiaoqi_target_index_from_key：把数字键或字母键转换为零开始目标下标，无法识别时返回 -1。
  function tiaoqi_target_index_from_key(tiaoqi_key) {
    if (/^[1-9]$/.test(tiaoqi_key)) return Number(tiaoqi_key) - 1;
    const tiaoqi_upper_key = tiaoqi_key.toUpperCase(); // tiaoqi_upper_key：统一后的大写目标字母。
    if (/^[A-Z]$/.test(tiaoqi_upper_key)) return 9 + tiaoqi_upper_key.charCodeAt(0) - "A".charCodeAt(0);
    return -1;
  }
  ...
  function tiaoqi_draw() {
    ...
    const tiaoqi_targets = tiaoqi_selected === null ? [] : tiaoqi_legal_targets(tiaoqi_state, tiaoqi_state.side, tiaoqi_selected); // tiaoqi_targets：当前选中棋子的合法落点。
    for (let tiaoqi_index = 0; tiaoqi_index < tiaoqi_points.length; tiaoqi_index += 1) {
      ...
      const tiaoqi_goal_owner = tiaoqi_state.phase === "setup" ? -1 : tiaoqi_goal_owner_for_arm(tiaoqi_state, tiaoqi_arm); // tiaoqi_goal_owner：把当前外臂作为最终区域的玩家。
      const tiaoqi_arm_color = tiaoqi_goal_owner >= 0
        ? tiaoqi_player_colors[tiaoqi_goal_owner]
        : (tiaoqi_state.phase === "setup" && tiaoqi_arm >= 0 ? tiaoqi_player_colors[tiaoqi_arm] : null); // tiaoqi_arm_color：按实际到达者映射后的终点区域颜色。
      tiaoqi_context.beginPath();
      tiaoqi_context.arc(tiaoqi_point.x, tiaoqi_point.y, 10, 0, Math.PI * 2);
      tiaoqi_context.fillStyle = tiaoqi_arm_color ? `${tiaoqi_arm_color}2b` : "rgb(225,230,236)";
      tiaoqi_context.fill();
      tiaoqi_context.strokeStyle = tiaoqi_arm_color ? `${tiaoqi_arm_color}88` : "rgb(183,194,205)";
      tiaoqi_context.lineWidth = 1.4;
      tiaoqi_context.stroke();
      const tiaoqi_target_index = tiaoqi_targets.indexOf(tiaoqi_index); // tiaoqi_target_index：当前落点在全部合法目标中的零开始下标。
      const tiaoqi_target_text = tiaoqi_target_label(tiaoqi_target_index); // tiaoqi_target_text：当前落点显示的数字或大写字母。
      if (tiaoqi_target_text) {
        tiaoqi_context.beginPath();
        tiaoqi_context.arc(tiaoqi_point.x, tiaoqi_point.y, 13, 0, Math.PI * 2);
        tiaoqi_context.fillStyle = "rgba(45,128,101,.16)";
        tiaoqi_context.fill();
        tiaoqi_context.strokeStyle = "#2d8065";
        tiaoqi_context.lineWidth = 3;
        tiaoqi_context.stroke();
        tiaoqi_context.fillStyle = "#1d684f";
        tiaoqi_context.font = "bold 11px sans-serif";
        tiaoqi_context.textAlign = "center";
        tiaoqi_context.textBaseline = "middle";
        tiaoqi_context.fillText(tiaoqi_target_text, tiaoqi_point.x, tiaoqi_point.y);
      }
  ...
  function tiaoqi_render_controls() {
    ...
    if (tiaoqi_state.over) {
      ...
      tiaoqi_rows.sort((tiaoqi_left, tiaoqi_right) => tiaoqi_left.rank - tiaoqi_right.rank).forEach(tiaoqi_entry => {
        ...
        const tiaoqi_suffix = tiaoqi_entry.out
          ? `　${tiaoqi_elimination_label(tiaoqi_state, tiaoqi_entry.player)}，完成${tiaoqi_state.eliminationProgress?.[tiaoqi_entry.player] || 0}/${tiaoqi_state.settings.pieceCount}`
    ...
    for (let tiaoqi_player = 0; tiaoqi_player < tiaoqi_state.settings.playerCount; tiaoqi_player += 1) {
      ...
      const tiaoqi_detail_text = tiaoqi_state.eliminated?.[tiaoqi_player]
        ? `${tiaoqi_elimination_label(tiaoqi_state, tiaoqi_player)}  第${tiaoqi_rank}名  完成${tiaoqi_finished}/${tiaoqi_state.settings.pieceCount}`
        : (tiaoqi_rank > 0
    ...
    const tiaoqi_percentages = tiaoqi_analysis_percentages(tiaoqi_state); // tiaoqi_percentages：复刻本地 C++ 局势分析并精确归一化后的整数百分比。
    ...
    const tiaoqi_targets = tiaoqi_selected === null ? [] : tiaoqi_legal_targets(tiaoqi_state, tiaoqi_state.side, tiaoqi_selected).slice(0, 35); // tiaoqi_targets：键盘 1-9、A-Z 可选择的合法落点。
    const tiaoqi_numbers_text = tiaoqi_selected === null
      ...
      : `可走位置编号：${tiaoqi_targets.map((_, tiaoqi_index) => tiaoqi_target_label(tiaoqi_index)).join("  ")}　0 返回`; // tiaoqi_numbers_text：随选棋或选落点阶段切换的键盘编号和字母说明。
  ...
  function tiaoqi_robot_move() {
    let tiaoqi_candidates = []; // tiaoqi_candidates：包含终点区撤离风险和综合分数的全部候选移动。
    for (const tiaoqi_move of tiaoqi_all_moves(tiaoqi_state, tiaoqi_state.side)) {
      ...
      const tiaoqi_from_risk = tiaoqi_opponent_zone_risk(tiaoqi_state, tiaoqi_state.side, tiaoqi_move.from); // tiaoqi_from_risk：移动前滞留在他人终点区的风险。
      const tiaoqi_target_risk = tiaoqi_opponent_zone_risk(tiaoqi_state, tiaoqi_state.side, tiaoqi_move.to); // tiaoqi_target_risk：移动后仍处于他人终点区的风险。
      const tiaoqi_zone_layer = tiaoqi_opponent_zone_layer(tiaoqi_state, tiaoqi_state.side, tiaoqi_move.from); // tiaoqi_zone_layer：原位置在他人终点区内从角尖向出口的层号。
      ...
      if (tiaoqi_target_risk === 3) tiaoqi_score -= 50000;
      else if (tiaoqi_target_risk === 2) tiaoqi_score -= 26000;
      else if (tiaoqi_target_risk === 1) tiaoqi_score -= 4200;
      if (tiaoqi_from_risk > tiaoqi_target_risk) {
        tiaoqi_score += tiaoqi_from_risk === 3 ? 22000 : (tiaoqi_from_risk === 2 ? 11000 : 4500);
      }
      tiaoqi_candidates.push({
        move: tiaoqi_move,
        score: tiaoqi_score,
        fromRisk: tiaoqi_from_risk,
        targetRisk: tiaoqi_target_risk,
        zoneLayer: tiaoqi_zone_layer,
      });
    }
    if (!tiaoqi_candidates.length) return null;
    const tiaoqi_highest_risk = Math.max(...tiaoqi_candidates.map(tiaoqi_candidate => tiaoqi_candidate.fromRisk)); // tiaoqi_highest_risk：当前可动棋子中最高的终点区滞留风险。
    if (tiaoqi_highest_risk >= 3) {
      const tiaoqi_direct_exits = tiaoqi_candidates.filter(tiaoqi_candidate => tiaoqi_candidate.fromRisk >= 3 && tiaoqi_candidate.targetRisk === 0); // tiaoqi_direct_exits：高危棋子本步可直接离开他人终点区的移动。
      if (tiaoqi_direct_exits.length) {
        tiaoqi_candidates = tiaoqi_direct_exits;
      } else {
        const tiaoqi_emergency = tiaoqi_candidates.filter(tiaoqi_candidate => tiaoqi_candidate.fromRisk >= 3); // tiaoqi_emergency：高危终点区内仍可移动的棋子。
        const tiaoqi_deepest_layer = Math.min(...tiaoqi_emergency.map(tiaoqi_candidate => tiaoqi_candidate.zoneLayer)); // tiaoqi_deepest_layer：最靠近区域角尖、最需要先撤离的层号。
        tiaoqi_candidates = tiaoqi_emergency.filter(tiaoqi_candidate => tiaoqi_candidate.zoneLayer === tiaoqi_deepest_layer);
      }
    } else if (tiaoqi_highest_risk > 0) {
      const tiaoqi_evacuation = tiaoqi_candidates.filter(tiaoqi_candidate => tiaoqi_candidate.fromRisk > tiaoqi_candidate.targetRisk); // tiaoqi_evacuation：能降低终点区滞留风险的移动。
      if (tiaoqi_evacuation.length) {
        const tiaoqi_deepest_layer = Math.min(...tiaoqi_evacuation.map(tiaoqi_candidate => tiaoqi_candidate.zoneLayer)); // tiaoqi_deepest_layer：优先撤离的最深区域层号。
        tiaoqi_candidates = tiaoqi_evacuation.filter(tiaoqi_candidate => tiaoqi_candidate.zoneLayer === tiaoqi_deepest_layer);
      }
    }
    tiaoqi_candidates.sort((tiaoqi_left, tiaoqi_right) => tiaoqi_right.score - tiaoqi_left.score);
    const tiaoqi_best_score = tiaoqi_candidates[0].score; // tiaoqi_best_score：风险筛选后的最高评分。
    const tiaoqi_near_best = tiaoqi_candidates.filter(tiaoqi_candidate => tiaoqi_candidate.score >= tiaoqi_best_score - 260).slice(0, 5); // tiaoqi_near_best：保持棋力的近似最优候选，允许不同对局产生变化。
    return tiaoqi_near_best[Math.floor(Math.random() * tiaoqi_near_best.length)].move;
  }
  ...
  function tiaoqi_commit_move(tiaoqi_move) {
    ...
    const tiaoqi_expected_owner = tiaoqi_state.side + 1; // tiaoqi_expected_owner：当前行动玩家在棋盘数组中的编码。
    if (tiaoqi_state.board[tiaoqi_move.from] !== tiaoqi_expected_owner
      || tiaoqi_state.board[tiaoqi_move.to]
      || !tiaoqi_legal_targets(tiaoqi_state, tiaoqi_state.side, tiaoqi_move.from).includes(tiaoqi_move.to)) return;
    ...
    const tiaoqi_zone_violators = tiaoqi_protected_zone_violators(tiaoqi_state); // tiaoqi_zone_violators：六成保护生效后仍占据他人终点区的玩家。
    if (tiaoqi_zone_violators.length) {
      tiaoqi_eliminate_players(tiaoqi_zone_violators, "终点区未及时离开判负", { turnOwner: tiaoqi_player });
      return;
    }
  ...
  function tiaoqi_schedule_robot() {
    ...
    if (tiaoqi_destroyed || tiaoqi_state.phase !== "play" || tiaoqi_state.over) return;
    const tiaoqi_zone_violators = tiaoqi_protected_zone_violators(tiaoqi_state); // tiaoqi_zone_violators：读取旧存档后仍滞留在受保护终点区的违规玩家。
    if (tiaoqi_zone_violators.length) {
      tiaoqi_eliminate_players(tiaoqi_zone_violators, "终点区未及时离开判负", { turnOwner: tiaoqi_state.side });
      return;
    }
    if (tiaoqi_state.eliminated?.[tiaoqi_state.side] || tiaoqi_state.completed?.[tiaoqi_state.side] || !tiaoqi_is_robot(tiaoqi_state, tiaoqi_state.side)) return;
  ...
  // tiaoqi_eliminate_players：一次清除所有违规玩家，保存固定原因和排名后继续剩余对局。
  function tiaoqi_eliminate_players(tiaoqi_players, tiaoqi_reason, { turnOwner: tiaoqi_turn_owner = tiaoqi_state.side } = {}) {
    if (tiaoqi_destroyed || tiaoqi_state.phase !== "play" || tiaoqi_state.over) return;
    const tiaoqi_valid_players = [...new Set(tiaoqi_players)].filter(tiaoqi_player => (
      Number.isInteger(tiaoqi_player)
      && tiaoqi_player >= 0
      && tiaoqi_player < tiaoqi_state.settings.playerCount
      && !tiaoqi_state.eliminated?.[tiaoqi_player]
      && !tiaoqi_state.completed?.[tiaoqi_player]
    )); // tiaoqi_valid_players：本次真正需要新增判负的玩家。
    if (!tiaoqi_valid_players.length) return;
    window.clearTimeout(tiaoqi_robot_timer);
    for (const tiaoqi_player of tiaoqi_valid_players) {
      tiaoqi_state.eliminationProgress[tiaoqi_player] = tiaoqi_goal_count(tiaoqi_state, tiaoqi_player);
      tiaoqi_state.eliminationReason[tiaoqi_player] = tiaoqi_reason;
      tiaoqi_state.eliminated[tiaoqi_player] = true;
      tiaoqi_state.eliminationOrder.push(tiaoqi_player);
    }
    const tiaoqi_owner_codes = new Set(tiaoqi_valid_players.map(tiaoqi_player => tiaoqi_player + 1)); // tiaoqi_owner_codes：需要从棋盘清除的玩家编码。
    tiaoqi_state.board = tiaoqi_state.board.map(tiaoqi_owner => tiaoqi_owner_codes.has(tiaoqi_owner) ? tiaoqi_empty : tiaoqi_owner);
    tiaoqi_selected = null;
    tiaoqi_pending_move = null;
    tiaoqi_hint_move = null;
    if (!tiaoqi_finish_if_decided()) {
      tiaoqi_state.side = tiaoqi_next_active_player(tiaoqi_state, tiaoqi_turn_owner);
      tiaoqi_state.stepRemaining = tiaoqi_state.settings.stepSeconds;
    }
    tiaoqi_draw();
    tiaoqi_update_panel(`${tiaoqi_valid_players.map(tiaoqi_player => tiaoqi_player_names[tiaoqi_player]).join("、")}${tiaoqi_reason}`);
    tiaoqi_services.save(qilei_deep_copy(tiaoqi_state));
    tiaoqi_schedule_robot();
  }

  // tiaoqi_eliminate_player：把单名玩家判负转交给统一批量清除流程。
  function tiaoqi_eliminate_player(tiaoqi_player, tiaoqi_reason) {
    tiaoqi_eliminate_players([tiaoqi_player], tiaoqi_reason, { turnOwner: tiaoqi_player });
  }
  ...
  function tiaoqi_handle_pointer(tiaoqi_event) {
    ...
    if (!tiaoqi_legal_targets(tiaoqi_state, tiaoqi_state.side, tiaoqi_selected).includes(tiaoqi_index)) return;
  ...
  function tiaoqi_handle_keyboard(tiaoqi_event) {
    ...
    if (tiaoqi_selected === null) {
      ...
      tiaoqi_event.preventDefault();
      const tiaoqi_number = Number(tiaoqi_event.key); // tiaoqi_number：选棋阶段使用的零开始棋子编号。
    ...
    const tiaoqi_target_index = tiaoqi_target_index_from_key(tiaoqi_event.key); // tiaoqi_target_index：1-9 或 A-Z 对应的零开始目标下标。
    if (tiaoqi_target_index < 0) return;
    tiaoqi_event.preventDefault();
    const tiaoqi_targets = tiaoqi_legal_targets(tiaoqi_state, tiaoqi_state.side, tiaoqi_selected).slice(0, 35); // tiaoqi_targets：一至九和 A-Z 对应的合法落点。
    const tiaoqi_target = tiaoqi_targets[tiaoqi_target_index]; // tiaoqi_target：按键对应的实际孔位。
    if (!Number.isInteger(tiaoqi_target)) return;
    const tiaoqi_target_text = tiaoqi_target_label(tiaoqi_target_index); // tiaoqi_target_text：本次选择的可见目标标签。
    ...
    if (tiaoqi_pending_move?.from === tiaoqi_candidate.from && tiaoqi_pending_move?.to === tiaoqi_candidate.to) {
      ...
      tiaoqi_pending_move = tiaoqi_candidate;
      tiaoqi_draw();
      tiaoqi_update_panel(`已选择 ${tiaoqi_target_text} 号落点，再按一次确认`);
    }
  }
