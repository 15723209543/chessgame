...
#include "zhongguoxiangqi_logger.h"
#include "zhongguoxiangqi_ui.h"
...
{
    return std::filesystem::exists(folder / L"chessgame.vcxproj");
}
...
static std::wstring getenginefolder()
{
    ...
    if (!projectpath.empty())
    {
        return (projectpath / L"engines" / L"zhongguoxiangqi").wstring();
    }
    ...
    return (std::filesystem::current_path() / L"engines" / L"zhongguoxiangqi").wstring();
}
